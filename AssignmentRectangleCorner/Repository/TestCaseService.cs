...
namespace AssignmentRectangleCorner.Repository
{
    public class TestCaseService : ITestCaseRepository
    {
            ...
            for (int i = 0; i < row; i++)
            {
                for (int j = 0; j < col; j++)
                {
                    for (int k = i+1; k < row; k++)
                    {
                        ...
                        {
                            float currMin = new List<float>() { matrix[i][j], matrix[i][l], matrix[k][j], matrix[k][l] }.Min();
                            maxValue = Math.Max(maxValue, currMin);
                        }
                    }
                }
            }
        ...
        {
            var data = dbcontext.TestCases.ToList();
            dbcontext.TestCases.RemoveRange(data);
            dbcontext.SaveChanges();
        }
